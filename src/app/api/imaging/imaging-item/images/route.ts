...
export async function GET(req: Request) {
  ...
  if (rows.length > 0) {
    const signed = await signImagingItemImages(admin, rows);
    if (signed.error) return NextResponse.json({ error: signed.error }, { status: 500 });
    return NextResponse.json({ ok: true, images: signed.images });
  }
