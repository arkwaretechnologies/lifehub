...
Your official imaging results are now ready for pick-up at the clinic.
...
export async function POST(req: Request) {
  ...
  if (!requestRow) return NextResponse.json({ error: "Imaging request not found." }, { status: 404 });

  const resultSmsSentAt =
    String((requestRow as { result_sms_sent_at?: string | null }).result_sms_sent_at ?? "").trim() || null;
  ...
  const { data: itemRows, error: itemErr } = await admin
    ...
    .select("findings, remarks")
    .eq("imaging_request_id", imagingRequestId);
  if (itemErr) return NextResponse.json({ error: itemErr.message }, { status: 500 });

  const items = (itemRows ?? []) as Array<{ findings?: string | null; remarks?: string | null }>;
  if (items.length === 0) {
    return NextResponse.json({ error: "No imaging request items found.", code: "notReady" }, { status: 400 });
  }
  if (!items.some((it) => imagingItemHasPrintableResult(it))) {
    return NextResponse.json(
  ...
  if (patientId == null) {
    return NextResponse.json({ error: "Patient not found for this imaging request.", code: "missingContact" }, { status: 400 });
  }
