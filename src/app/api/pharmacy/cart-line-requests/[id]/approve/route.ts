...
import { NOTIFICATION_TYPE_PHARMACY_CART_LINE } from "@/lib/pharmacyLineRequestServer";
...
export async function POST(req: Request, context: RouteContext) {
  ...
  if (!session.ok || !userCanApprovePharmacyLineRequests(session.menuAccess)) {
    return NextResponse.json({ error: "You do not have permission to approve line requests." }, { status: 403 });
  }
  ...
  const current = row as { status: string };
  if (current.status !== "pending") {
    return NextResponse.json({ error: `Request is already ${current.status}.` }, { status: 409 });
  }
  ...
  await db
    .from("notifications")
    .update({ read_at: now })
    .eq("type", NOTIFICATION_TYPE_PHARMACY_CART_LINE)
