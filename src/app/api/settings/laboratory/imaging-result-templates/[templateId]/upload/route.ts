...
import {
  IMAGING_RESULT_TEMPLATE_PDF_MAX_BYTES,
  IMAGING_RESULT_TEMPLATES_TABLE,
  IMAGING_RESULTS_TEMPLATES_RELATIVE_DIR,
...
export async function POST(
  ...
  if (file.size <= 0 || file.size > IMAGING_RESULT_TEMPLATE_PDF_MAX_BYTES) {
    return NextResponse.json(
      { error: `PDF must be between 1 byte and ${IMAGING_RESULT_TEMPLATE_PDF_MAX_BYTES} bytes.` },
      { status: 400 },
    );
  }
  ...
  const dir = path.join(process.cwd(), IMAGING_RESULTS_TEMPLATES_RELATIVE_DIR);
  await mkdir(dir, { recursive: true });
  const target = imagingResultTemplatePdfAbsolutePath(fileName);
