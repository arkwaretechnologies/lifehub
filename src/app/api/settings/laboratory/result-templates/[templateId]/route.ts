...
export async function PATCH(
  ...
  if (body.code !== undefined) {
    const c = normalizeLabResultTemplateCode(body.code);
  ...
  if (body.signature_layout !== undefined) {
    const layoutParsed = parseTemplateSignatureLayoutInput(body.signature_layout);
    if (!layoutParsed.ok) {
...
export async function DELETE(
  ...
  const inUse = (tests ?? []).some((t) => {
    const csv = String((t as { results_template_code?: string }).results_template_code ?? "");
    return csv
      .split(",")
  ...
  if (inUse) {
    return NextResponse.json(
      {
        error:
          "Cannot delete a template that is assigned to lab tests. Clear or change those tests first.",
      },
      { status: 409 },
    );
  }

  const { error } = await db.from(LAB_RESULT_TEMPLATES_TABLE).delete().eq("id", templateId);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
