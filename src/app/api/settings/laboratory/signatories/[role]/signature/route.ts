...
import {
  fetchLabSignatorySignaturePath,
  setLabSignatorySignaturePath,
} from "@/lib/labResultSignatories";
import { getBearerSessionUserId } from "@/lib/requireSession";
import {
  buildLabSignatoryStoragePath,
  parseLabSignatoryRole,
...
export async function POST(
  ...
  const { db, role } = ctx as { db: NonNullable<ReturnType<typeof supabaseAdminClient>>; role: LabSignatureRole };
  ...
  const { path: previousPath } = await fetchLabSignatorySignaturePath(db, role);
  const inputBuffer = Buffer.from(await file.arrayBuffer());
  const optimized = await optimizeSignatureBuffer(inputBuffer, file.name);
  const storagePath = buildLabSignatoryStoragePath(role, optimized.ext);
  ...
  const { signatories, error } = await setLabSignatorySignaturePath(db, role, storagePath);
  if (error) {
...
export async function DELETE(
  ...
  const { signatories, error } = await setLabSignatorySignaturePath(db, role, null);
...
export async function GET(
  ...
  const { path, error: pathErr } = await fetchLabSignatorySignaturePath(db, role);
